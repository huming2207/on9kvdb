...
#define EXPECT_EQ(expected, actual)                                           \
    ...
    static const constexpr uint32_t data_file_count =
        on9kvdb_def::wal_file_count + table_count;
    ...
    bool select_manifest(
        const uint8_t slots[on9kvdb_def::manifest_slot_count]
                           [on9kvdb_def::manifest_record_size],
        ...
        for (uint32_t slot = 0;
            ...
            const on9kvdb_def::format_status status =
                on9kvdb_def::decode_manifest_record(
                    slots[slot], on9kvdb_def::manifest_record_size,
                    &candidate);
    ...
    struct model_file {
        bool exists = false;
        uint8_t working[on9kvdb_def::identity_slot_count]
                       [on9kvdb_def::file_identity_size] = {};
        uint8_t durable[on9kvdb_def::identity_slot_count]
                       [on9kvdb_def::file_identity_size] = {};
    };

    struct model_image {
        bool manifest_exists = false;
        uint8_t manifest_working[on9kvdb_def::manifest_slot_count]
                                [on9kvdb_def::manifest_record_size] = {};
        uint8_t manifest_durable[on9kvdb_def::manifest_slot_count]
                                [on9kvdb_def::manifest_record_size] = {};
        model_file files[data_file_count] = {};

        void reset()
        {
            memcpy(manifest_working, manifest_durable,
                   sizeof(manifest_working));
            for (uint32_t idx = 0; idx < data_file_count; idx += 1) {
                memcpy(files[idx].working, files[idx].durable,
                       sizeof(files[idx].working));
            }
        }
    };
    ...
    bool write_manifest_event(
        model_image *image,
        const on9kvdb_def::manifest_record &record,
        event_injector *injector)
    {
        const uint32_t slot = static_cast<uint32_t>(
            (record.generation - 1U) %
            on9kvdb_def::manifest_slot_count);
        EXPECT_TRUE(on9kvdb_def::encode_manifest_record(
            image->manifest_working[slot],
            on9kvdb_def::manifest_record_size, record));
        return injector->interrupted();
    }

    bool sync_manifest_event(
        model_image *image, event_injector *injector)
    {
        memcpy(image->manifest_durable,
               image->manifest_working,
               sizeof(image->manifest_durable));
        return injector->interrupted();
    }

    bool write_identity_event(
        model_image *image, uint32_t file_index,
        uint32_t copy_slot,
        const on9kvdb_def::file_identity &identity,
        event_injector *injector)
    {
        EXPECT_TRUE(on9kvdb_def::encode_file_identity(
            image->files[file_index].working[copy_slot],
            on9kvdb_def::file_identity_size, identity));
        return injector->interrupted();
    }

    bool sync_identity_event(
        model_image *image, uint32_t file_index,
        event_injector *injector)
    {
        memcpy(image->files[file_index].durable,
               image->files[file_index].working,
               sizeof(image->files[file_index].durable));
        return injector->interrupted();
    }

    model_result run_provisioning(
        model_image *image, event_injector *injector)
    {
        const on9kvdb_def::storage_geometry geometry =
            make_geometry();
        static const constexpr uint64_t database_id =
        ...
        if (!image->manifest_exists) {
            ...
            valid_manifest_copies =
                on9kvdb_def::manifest_slot_count;
        } else if (!select_manifest(
                       image->manifest_working, &state,
                       &valid_manifest_copies)) {
        ...
        for (uint32_t file_index = 0;
             file_index < data_file_count; file_index += 1) {
            model_file &file = image->files[file_index];
            file.exists = true;
            const bool wal =
                file_index < on9kvdb_def::wal_file_count;
            const on9kvdb_def::file_kind kind = wal ?
                on9kvdb_def::file_kind::wal :
                on9kvdb_def::file_kind::table;
            const uint32_t slot = wal ?
                file_index :
                file_index - on9kvdb_def::wal_file_count;
            const uint64_t file_size = wal ?
                geometry.wal_size : geometry.table_size;
            ...
            for (uint32_t copy_slot = 0;
                ...
                identity.slot = slot;
                identity.kind = kind;
                if (write_identity_event(
                        image, file_index, copy_slot,
                        identity, injector)) {
                    return model_result::interrupted;
                }
                if (sync_identity_event(
                        image, file_index, injector)) {
                    return model_result::interrupted;
                }
            }
        }
    ...
    void test_geometry_and_records()
    {
        ...
        mismatch.provisioned_size =
            mismatch.manifest_size +
            static_cast<uint64_t>(mismatch.wal_count) *
                mismatch.wal_size +
            static_cast<uint64_t>(mismatch.table_count) *
                mismatch.table_size;
        EXPECT_TRUE(!on9kvdb_def::storage_geometry_equal(
            geometry, mismatch));
        EXPECT_TRUE(
            on9kvdb_def::validate_storage_geometry(mismatch));
        ...
        EXPECT_TRUE(on9kvdb_def::storage_geometry_equal(
            record.geometry, decoded.geometry));
        ...
        uint8_t identity_bytes[
            on9kvdb_def::file_identity_size] = {};
        EXPECT_TRUE(on9kvdb_def::encode_file_identity(
            identity_bytes, sizeof(identity_bytes), identity));
    ...
    void test_every_provisioning_write_and_sync()
    {
        ...
        EXPECT_EQ(model_result::complete,
                  run_provisioning(
                      &baseline, &baseline_injector));
        const int event_count =
            baseline_injector.event_count;
        EXPECT_TRUE(event_count > 0);

        for (int stop_after = 0;
             stop_after < event_count; stop_after += 1) {
            model_image image = {};
            event_injector first = {};
            first.stop_after = stop_after;
            EXPECT_EQ(model_result::interrupted,
                      run_provisioning(&image, &first));
            image.reset();

            event_injector recovery = {};
            const model_result recovered =
                run_provisioning(&image, &recovery);
            if (stop_after == 0) {
            ...
            on9kvdb_def::manifest_record selected = {};
            EXPECT_TRUE(select_manifest(
                image.manifest_durable, &selected, nullptr));
            EXPECT_EQ(
