...
#define EXPECT_EQ(expected, actual)                                           \
    ...
    public:
        ...
        bool commit()
        {
            ...
            for (size_t idx = 0; idx < entry_capacity; idx += 1) {
                ...
                model_entry *destination =
                    find_or_create(next, staged[idx].key);
    ...
    private:
        ...
        static const model_entry *find(
            const model_entry *entries, const char *key)
        {
            for (size_t idx = 0; idx < entry_capacity; idx += 1) {
                if (entries[idx].used &&
                    strcmp(entries[idx].key, key) == 0) {
                    return &entries[idx];
                }
            }
            return nullptr;
        ...
        static model_entry *find_or_create(
            model_entry *entries, const char *key)
        {
    ...
    void test_checked_arithmetic()
    {
        ...
        EXPECT_TRUE(!on9kvdb_def::checked_add_size(
            std::numeric_limits<size_t>::max(), 1, &size_result));

        EXPECT_TRUE(on9kvdb_def::checked_mul_size(7, 9, &size_result));
        EXPECT_EQ(static_cast<size_t>(63), size_result);
        EXPECT_TRUE(!on9kvdb_def::checked_mul_size(
            std::numeric_limits<size_t>::max(), 2, &size_result));

        EXPECT_TRUE(on9kvdb_def::checked_align_up_size(
            17, 8, &size_result));
        ...
        EXPECT_TRUE(!on9kvdb_def::checked_add_u64(
            UINT64_MAX, 1, &u64_result));
        EXPECT_TRUE(!on9kvdb_def::checked_mul_u64(
            UINT64_MAX, 2, &u64_result));
    }
    ...
    {
        uint8_t encoded[14] = {};
        EXPECT_TRUE(on9kvdb_def::write_u16_le(
            encoded, sizeof(encoded), 0, 0x1234));
        EXPECT_TRUE(on9kvdb_def::write_u32_le(
            encoded, sizeof(encoded), 2, 0x89abcdefUL));
        EXPECT_TRUE(on9kvdb_def::write_u64_le(
            encoded, sizeof(encoded), 6, UINT64_C(0x0123456789abcdef)));
        ...
        uint32_t value32 = 0;
        uint64_t value64 = 0;
        EXPECT_TRUE(on9kvdb_def::read_u16_le(
            encoded, sizeof(encoded), 0, &value16));
        EXPECT_TRUE(on9kvdb_def::read_u32_le(
            encoded, sizeof(encoded), 2, &value32));
        EXPECT_TRUE(on9kvdb_def::read_u64_le(
            encoded, sizeof(encoded), 6, &value64));
        ...
        EXPECT_TRUE(!on9kvdb_def::write_u64_le(
            encoded, sizeof(encoded), 7, value64));
    }

    void test_crc()
    {
        static const constexpr uint8_t input[] = "123456789";
        EXPECT_EQ(UINT32_C(0xcbf43926),
                  on9kvdb_def::calc_crc32(input, sizeof(input) - 1));
        EXPECT_EQ(UINT32_C(0),
                  on9kvdb_def::calc_crc32(nullptr, 0));
    }

    void test_handle_codec()
    {
        const uint32_t value =
            on9kvdb_def::make_handle_value(7, 9);
        EXPECT_TRUE(value != 0);

        uint16_t slot = 0;
        uint16_t generation = 0;
        EXPECT_TRUE(on9kvdb_def::decode_handle_value(
        ...
        EXPECT_EQ(UINT32_C(0),
                  on9kvdb_def::make_handle_value(UINT16_MAX, 1));
        EXPECT_EQ(UINT32_C(0),
                  on9kvdb_def::make_handle_value(0, 0));
        EXPECT_TRUE(!on9kvdb_def::decode_handle_value(
            0, &slot, &generation));
    }
    ...
    void test_file_prefix()
    {
        ...
        EXPECT_TRUE(on9kvdb_def::encode_file_prefix(
            encoded, sizeof(encoded),
            on9kvdb_def::wal_magic,
            on9kvdb_def::file_kind::wal,
            0x1234, UINT64_C(0x0102030405060708),
            UINT32_C(0x11223344)));
        ...
        EXPECT_EQ(on9kvdb_def::format_status::ok,
                  on9kvdb_def::decode_file_prefix(
                      encoded, sizeof(encoded),
                      on9kvdb_def::wal_magic,
                      on9kvdb_def::file_kind::wal, &decoded));
